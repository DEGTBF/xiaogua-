...
async function mainLoop() {
    ...
    while (true) {
        ...
        if (currentBinancePrice === 0 || gamePrice === 0) {
            ...
            await new Promise(resolve => setTimeout(resolve, 100)); // 等待0.1秒再检查
            continue;
        }

        // 计算价格差
        const priceDifference = currentBinancePrice - gamePrice;

        // 输出当前价格信息
        console.log(`当前币安价格: ${currentBinancePrice}, 当前游戏价格: ${gamePrice}, 价格差: ${priceDifference}`);

        // 保存当前价格到之前的价格数组
        previousBinancePrices.push(currentBinancePrice);
        if (previousBinancePrices.length > 20) {
            previousBinancePrices.shift(); // 保持数组长度为20
        }

        // 确定下注方向
        let betDirection;
        const priceThreshold = 10; // 平台价格阈值
        const currentPriceThreshold = 8; // 币安价格阈值

        // 判断是否下注
        if (previousBinancePrices.length >= 20 && previousBinancePrices.some(price => currentBinancePrice >= price + currentPriceThreshold) && currentBinancePrice >= gamePrice + priceThreshold) {
            betDirection = 1; // 下注上涨
        } else if (previousBinancePrices.length >= 20 && previousBinancePrices.some(price => currentBinancePrice <= price - currentPriceThreshold) && currentBinancePrice <= gamePrice - priceThreshold) {
            betDirection = 2; // 下注下跌
        } else {
            console.log("不下注，等待机会。");
            await new Promise(resolve => setTimeout(resolve, 100)); // 每0.1秒重新获取
            continue; // 继续下一次循环
        }

        // 检查下注金额 (85%-95%)
        const betAmount = Math.floor(currentBalance * (Math.random() * (0.10) + 0.85)); // 计算下注金额，范围85%-95%
        ...
        // 输出当前余额信息（此处代码已调整，根据你的要求）
        console.log(`当前余额: ${currentBalance}, 总下注次数: ${totalBets}`);
