...
const binancePrices = []; // 存储获取的币安价格
const maxPricesCount = 100; // 限制存储的最大价格条数
...
ws.onmessage = function(event) {
    ...
    // 存储价格并限制数量
    binancePrices.push(currentBinancePrice);
    if (binancePrices.length > maxPricesCount) {
        binancePrices.shift(); // 保持数组大小不超过maxPricesCount
    }
};
...
async function mainLoop() {
    ...
    while (true) {
        ...
        // 输出当前币安价格和获取的条数
        console.log(`当前币安价格: ${currentBinancePrice}, 获取的币安价格条数: ${binancePrices.length}`);
        if (binancePrices.length > 0) {
            console.log(`最小币安价格: ${Math.min(...binancePrices)}, 最大币安价格: ${Math.max(...binancePrices)}`);
        }
        ...
        // 输出当前价格信息
        console.log(`当前游戏价格: ${gamePrice}, 价格差: ${priceDifference}`);

        // 确定下注方向
        let betDirection;
        const minBinancePrice = Math.min(...binancePrices);
        const maxBinancePrice = Math.max(...binancePrices);

        if (currentBinancePrice >= minBinancePrice + 8 && currentBinancePrice >= gamePrice + 10) {
            betDirection = 1; // 下注上涨
        } else if (currentBinancePrice <= maxBinancePrice - 8 && currentBinancePrice <= gamePrice - 10) {
            betDirection = 2; // 下注下跌
        } else {
