...
async function mainLoop() {
    ...
    while (true) {
        ...
        if (previousBinancePrices.length > 5) {
            previousBinancePrices.shift(); // 保持数组长度为5
        }
        ...
        // 判断是否下注
        if (previousBinancePrices.some(price => currentBinancePrice >= price + currentPriceThreshold) && currentBinancePrice >= gamePrice + priceThreshold) {
            betDirection = 1; // 下注上涨
        } else if (previousBinancePrices.some(price => currentBinancePrice <= price - currentPriceThreshold) && currentBinancePrice <= gamePrice - priceThreshold) {
            betDirection = 2; // 下注下跌
        } else {
        ...
        // 检查下注金额
        const betAmount = Math.floor(currentBalance * (Math.random() * (0.08) + 0.40)); // 计算下注金额，范围40%-48%
