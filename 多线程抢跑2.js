...
const binancePrices = []; // 用于存储最近的50条币安价格
...
ws.onmessage = function(event) {
    ...
    // 保持价格数组的长度为50
    if (binancePrices.length > 50) {
        binancePrices.shift(); // 移除最旧的一条
    }
};
...
async function mainLoop() {
    ...
    while (true) {
        ...
        if (currentBinancePrice === 0 || gamePrice === 0) {
            ...
            await new Promise(resolve => setTimeout(resolve, 10)); // 等待0.01秒再检查
            continue;
        }
        ...
        if (currentBinancePrice >= minBinancePrice + 8 && currentBinancePrice >= gamePrice + 10) {
            ...
            await new Promise(resolve => setTimeout(resolve, 10)); // 每0.01秒重新获取
            continue; // 继续下一次循环
        }
