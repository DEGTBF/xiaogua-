...
let binancePriceHistory = []; // 记录前5条币安价格
...
ws.onmessage = function(event) {
    ...
    // 保持历史价格只保留最新的5条
    if (binancePriceHistory.length > 5) {
        binancePriceHistory.shift();
    }
};
...
async function mainLoop() {
    ...
    while (true) {
        ...
        // 确定下注方向
        let betDirection;
        if (currentBinancePrice >= previousBinancePrice + 30) {
            betDirection = 1; // 下注上涨
        } else if (currentBinancePrice <= previousBinancePrice - 30) {
            betDirection = 2; // 下注下跌
        } else {
        ...
        // 检查下注金额，40%-48%的余额
        const betAmount = Math.floor(currentBalance * (Math.random() * (0.48 - 0.40) + 0.40)); 
