...
function calculateMovingAverage(data, period) {
    if (data.length < period) return null; // 数据不足以计算移动平均线
    const sum = data.slice(-period).reduce((acc, val) => acc + val, 0);
    return sum / period; // 返回移动平均值
}
...
async function mainLoop() {
    ...
    while (true) {
        ...
        // 将当前游戏价格加入历史数据
        priceHistory.push(gamePrice); // 存储游戏价格

        // 计算移动平均线
        const movingAverage = calculateMovingAverage(priceHistory, 5); // 计算5个周期的移动平均线

        // 确定下注方向
        let betDirection;
        if (movingAverage && gamePrice > movingAverage) {
            betDirection = 1; // 游戏价格大于移动平均线，下注上涨
        } else {
            betDirection = 2; // 否则下注下跌
        }

        // 输出当前阶段信息
        console.log(`当前币安价格: ${binancePrice}, 游戏价格: ${gamePrice}, 移动平均线: ${movingAverage}, 下注方向: ${betDirection === 1 ? "上涨" : "下跌"}`);
