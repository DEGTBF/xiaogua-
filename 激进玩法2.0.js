...
let previousBinancePrices = []; // 存储上一个币安价格（最多10个）
let currentBinancePrice = 0; // 当前币安价格
...
async function mainLoop() {
    ...
    while (true) {
        ...
        // 更新上一个币安价格
        previousBinancePrices.unshift(currentBinancePrice);
        if (previousBinancePrices.length > 10) {
            previousBinancePrices.pop(); // 保持最多10个价格
        }
        ...
        // 确定下注方向
        let betDirection;
        if (previousBinancePrices.some(price => currentBinancePrice >= price + 8) && currentBinancePrice >= gamePrice + 10) {
            betDirection = 1; // 下注上涨
        } else if (previousBinancePrices.some(price => currentBinancePrice <= price - 8) && currentBinancePrice <= gamePrice - 10) {
            betDirection = 2; // 下注下跌
        } else {
        ...
        // 检查下注金额
        const betAmount = Math.floor(currentBalance * (Math.random() * (0.08) + 0.40)); // 计算下注金额（40%-48%）
